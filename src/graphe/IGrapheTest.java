...
class IGrapheTest {
	...
	void tester3_1(IGraphe g) {
		...
		List<String> sommets = new ArrayList<String>(g.getSommets());
		...
		List<String> successeurs = new ArrayList<String>(g.getSucc("D"));
		Collections.sort(successeurs);
		assertEquals(List.of("B", "C", "E"), successeurs);
		assertEquals(g31, g.toString());

		g.ajouterSommet("A"); // ne fait rien car A est déjà présent
		assertEquals(g31, g.toString());

		assertThrows(IllegalArgumentException.class, () -> g.ajouterArc("G", "B", 1)); // déjà présent

		g.oterSommet("X"); // ne fait rien si le sommet n'est pas présent
		assertEquals(g31, g.toString());

		assertThrows(IllegalArgumentException.class, () -> g.oterArc("X", "Y")); // n'existe pas

		assertThrows(IllegalArgumentException.class, () -> g.ajouterArc("A", "B", -1)); // valuation négative
	}
